...
void LatticeBoltzmann::Start(double rho0, std::vector<double>& speed_x, std::vector<double>& speed_y){
  int ix, iy, i, n0, nv; double Ux0, Uy0;
...
void LatticeBoltzmann::Advection(std::vector<double>& speed_x, std::vector<double>& speed_y, int t){
  int ix, iy, i, ixnext, iynext, ixback, iyback,nv, n0, n0next; double Ux0, Uy0;
  for(ix=0;ix<Lx;ix++)      //para cada celda
    for(iy=0;iy<Ly;iy++)
      for(i=0;i<Q;i++){     //en cada direccion
...
int main(void){
  ...
  //Para leer el dato de viento de la posición [ix,iy] basta con hacer speed_x[n_viento(ix,iy)] y speed_y[n_viento(ix,iy)]
