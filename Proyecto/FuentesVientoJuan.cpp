...
public:
  ...
  void Start(double rho0, std::vector<double>& speed_x, std::vector<double>& speed_y);
  void Collision(std::vector<double>& speed_x, std::vector<double>& speed_y);
  void ImposeFields();
  void Advection(std::vector<double>& speed_x, std::vector<double>& speed_y, int t);
...
//Start
void LatticeBoltzmann::Start(double rho0, std::vector<double>& speed_x, std::vector<double>& speed_y){
  int ix, iy, i, n0; 
  for(ix=0; ix<Lx; ix++)    //para cada celda
    for(iy=0; iy<Ly; iy++)
      for(i=0; i<Q; i++){   //en cada direccion
        ...
        double Uy0=speed_y[n_viento(ix,iy)];
...
//Colision
void LatticeBoltzmann::Collision(std::vector<double>& speed_x, std::vector<double>& speed_y){
  ...
  for(ix=0;ix<Lx;ix++)      //para cada celda
    for(iy=0;iy<Ly;iy++){
      ...
      for(i=0;i<Q;i++){     //para cada vector de velocidad
        ...
        double Ux0=speed_x[n_viento(ix,iy)];
        double Uy0=speed_y[n_viento(ix,iy)];
        //double Ux0 = Jx(ix,iy,false)/rho0, Uy0 = Jy(ix,iy,false)/rho0;
...
//Adveccion
void LatticeBoltzmann::Advection(std::vector<double>& speed_x, std::vector<double>& speed_y, int t){
...
void read_data(std::vector<double>& speed_x, std::vector<double>& speed_y){
    ...
    while (std::getline(line_x,segment,',')){
        speed_x.push_back(std::stod(segment)/500.0);
    }
    while (std::getline(line_y,segment,',')){
        speed_y.push_back(std::stod(segment)/500.0);
    }
    data_x.close();
    data_y.close();
}
...
int main(void){
  ...
  Ondas.Start(rho0,speed_x,speed_y);
  //Evolucione
  for(t=1; t<tmax; t++){
    Ondas.Collision(speed_x,speed_y);
    Ondas.ImposeFields();
    Ondas.Advection(speed_x,speed_y,t);
